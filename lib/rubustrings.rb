...
class Rubustrings
  ...
  def self.log_output(level, file_name, line_number, xcode_output, message)
    if xcode_output
  ...
  def self.validate_localizable_string_file(file_name, use_xcode)
  ...
  def self.add_line_numbers(file_data)
    line_num = 0
    result = ''
  ...
  def self.open_and_read_file(file_name)
  ...
  def self.validate_format(line)
    localizable_strings_format_regex = /^\"((?:\\.|[^\\"])*?)\"\s=\s\"((?:\\.|[^\\"])*?)\";/
    localizable_strings_format_regex.match line
  end

  def self.validate_special_characters(translation_key, translation_value)
  ...
  def self.validate_special_beginning(translation_key, translation_value)
    beginning_regex = /^(?:\s|\n|\r)/
  ...
  def self.validate_special_ending(translation_key, translation_value)
    ending_regex = /(?:\s|\n|\r)$/
  ...
  def self.check_translation_length(translation_key, translation_value)
    translation_value.length / translation_key.length < 3
  end

  def self.validate_translation_line(file_name, line, use_xcode)
    line_number = 0
